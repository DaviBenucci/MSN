...
def test_conciliar_accepts_cliente_modelo_column(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(conciliador, "CACHE_ROOT", tmp_path / "cache")
    ...
    cliente_bytes = make_xlsx(
        pd.DataFrame(
            [
                {
                    "Modelo": "Toner HP Q2612A Preto",
                    "Quantidade": 7,
                    "Valor Unitário": 123.45,
                },
                {
                    "Modelo": "Toner Ricoh 3503H Ciano",
                    "Quantidade": 2,
                    "Valor Unitário": 88.0,
                },
            ]
